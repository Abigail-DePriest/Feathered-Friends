...
function HopefulsForm({ obj }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (obj.firebaseKey) {
      updateHopeful(formInput).then(() => router.push(`/hopefuls/${obj.firebaseKey}`));
    } else {
    ...
    <Form onSubmit={handleSubmit}>
      ...
      <FloatingLabel controlId="floatingSelect" label="Habitat">
        ...
        >
          ...
          <option value="option 1"> Forests </option>
          <option value="option 2"> Grasslands </option>
          <option value="option 3"> Mountains </option>
          <option value="option 4"> Rivers and Streams </option>
          <option value="option 5"> Farmlands and Agricultural Areas</option>
          <option value="option 6"> Urban Areas </option>
          <option value="option 7"> Wetlands </option>
          {
            hopefuls.map((hopeful) => (
              <option
                key={hopefuls.firebaseKey}
                value={hopefuls.firebaseKey}
              >
                {hopeful.birdName} {hopeful.seen}
              </option>
            ))
          }
        </Form.Select>
